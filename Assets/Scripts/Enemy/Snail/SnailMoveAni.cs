...
/// <summary>
/// 蜗牛移动时视觉 Transform 的 scale 正弦挤压动画（同鼹鼠 MoleMoveAni）。
/// </summary>
public class SnailMoveAni : MonoBehaviour
{
    [Tooltip("要变形的贴图 / 视觉 Transform")]
    public Transform visualTransform;

    [Tooltip("可选：同物体上的蜗牛，用于判断是否在移动")]
    public Snail2D snail;
    ...
    [Tooltip("位移低于此值视为未移动（无 Snail2D 时）")]
    ...
    private bool IsMoving()
    {
        if (snail != null)
        {
            ...
            return Vector2.Distance(snail.Position, snail.CurrentTarget) > 0.05f;
        }
